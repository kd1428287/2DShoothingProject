...
public:
	...
	void DrawTex_color(const KdTexture* tex, const Math::Rectangle& srcRect = { 0,0,0,0 }, Math::Color clr = { 1,1,1,1 }, const Math::Vector2& pivot = { 0.5, 0.5f })
	{
		assert(tex && tex->GetSRViewAddress());
		if (tex == nullptr)return;
		DrawTex(tex, 0, 0, srcRect.width, srcRect.height, &srcRect, &clr, pivot);
	}
	...
	void SetFlashValue(float flash) { m_flashValue = flash; }
...
private:
	...
	struct cbSprite {
		...
		float           m_flashValue = 0.0f; // フラッシュ値
		float           m_dummy[3];          // パディング
	};
	KdConstantBuffer<cbSprite>	m_cb0;

	float           m_flashValue = 0.0f;
